...
export async function POST(request: NextRequest) {
  ...
  // AI応答前にユーザー発言までを保存しておく(応答全文はストリーム完了後に
  // クライアントがPATCHで保存する。Vercelではストリーム完了後のサーバー処理に実行保証がない)
  ...
  if (sessionId) {
    ...
    if (existing) {
      await prisma.consultSession.update({
        ...
        data: { messages: cleanMessages, updatedAt: new Date() },
      });
    } else {
      sessionId = null;
    }
